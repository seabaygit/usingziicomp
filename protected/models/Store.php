...
class Store extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('manager_staff_id, address_id, last_update, store', 'required'),
			array('manager_staff_id, address_id', 'numerical', 'integerOnly'=>true),
			array('store', 'length', 'max'=>80),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('store_id, manager_staff_id, address_id, last_update, store', 'safe', 'on'=>'search'),
		);
	}
